          ...
          >
             {editingFolderId === folder.id ? (
                <div className="flex items-center gap-1 w-full" onClick={e => e.stopPropagation()}>
                    <input autoFocus className="w-full text-sm p-1 border border-indigo-300 rounded" value={editName} onChange={e => setEditName(e.target.value)} onKeyDown={e => e.key === 'Enter' && saveEdit()} onBlur={saveEdit} />
                </div>
             ) : (
                <>
                    ...
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        {folderStudies.length > 0 && (<button onClick={(e) => { e.stopPropagation(); onFolderExam(folder.id); }} className="p-1 text-purple-600 hover:bg-purple-100 rounded" title="Gerar Provão da Pasta"><GraduationCap className="w-3 h-3"/></button>)}
                        {isRenamable && (<button onClick={(e) => { e.stopPropagation(); startEditing(folder); }} className="p-1 hover:text-indigo-600 hover:bg-indigo-50 rounded" title="Renomear"><Edit className="w-3 h-3"/></button>)}
                        <button onClick={(e) => { e.stopPropagation(); setCreatingSubfolderIn(folder.id); setExpandedFolders(p => ({...p, [folder.id]: true})); }} className="p-1 hover:text-green-600 hover:bg-green-50 rounded" title="Nova Subpasta"><Plus className="w-3 h-3"/></button>
                        {isDeletable && (<button onClick={(e) => { e.stopPropagation(); onDeleteFolder(folder.id); }} className="p-1 hover:text-red-500 hover:bg-red-50 rounded" title="Excluir"><Trash className="w-3 h-3"/></button>)}
                    </div>
                </>
             )}
          </div>
            ...
            <div className="space-y-0.5">
               {creatingSubfolderIn === folder.id && (
                  <div className="flex items-center gap-2 p-1 bg-white rounded border border-gray-200 ml-6 my-1">
                     ...
                     <input autoFocus placeholder="Nome da subpasta..." className="text-sm p-1 border-none outline-none w-full bg-transparent" value={newSubfolderName} onChange={e => setNewSubfolderName(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleCreateSubfolder(folder.id)} />
               ...
               <div style={{ marginLeft: `${(depth + 1) * 12 + 8}px` }}>
                    {creatingStudyInFolder === folder.id ? (
                        <div className="flex flex-col gap-2 p-2 bg-white rounded border border-indigo-200 shadow-sm my-1">
                            <input autoFocus type="text" value={newStudyTitle} onChange={e => setNewStudyTitle(e.target.value)} placeholder="Nome do estudo..." className="text-sm p-1 border border-gray-200 rounded w-full" onKeyDown={e => e.key === 'Enter' && handleCreateStudy(folder.id)} />
                        ...
                        <button onClick={() => setCreatingStudyInFolder(folder.id)} className="w-full text-left flex items-center gap-2 px-2 py-1.5 text-xs text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded dashed-border"><Plus className="w-3 h-3"/> Novo Estudo aqui</button>
                    )}
               </div>

               {folderStudies.map(study => (
                 <div key={study.id} draggable onDragStart={(e) => handleDragStart(e, 'STUDY', study.id)} style={{ marginLeft: `${(depth + 1) * 12}px` }} onClick={() => onSelectStudy(study.id)} className={`group flex items-center justify-between px-2 py-1.5 rounded text-sm cursor-pointer border-l-2 ${activeStudyId === study.id ? 'bg-white text-indigo-700 font-semibold shadow-sm border-indigo-500' : 'border-transparent text-gray-600 hover:bg-gray-100 hover:border-gray-300'}`}>
                   <div className="flex items-center gap-2 truncate"><FileText className="w-3 h-3"/><span className="truncate">{study.title}</span></div>
                   <button onClick={(e) => { e.stopPropagation(); onDeleteStudy(study.id); }} className="opacity-0 group-hover:opacity-100 p-1 hover:text-red-500"><Trash className="w-3 h-3"/></button>
                 </div>
               ))}
            </div>
          )}
      ...
      <div className="p-4 border-t border-gray-200 bg-gray-50 space-y-3">
         {isCreatingRootFolder ? (
           <div className="flex gap-2 animate-in slide-in-from-bottom-2">
             <input autoFocus type="text" value={newRootFolderName} onChange={e => setNewRootFolderName(e.target.value)} placeholder="Nome da pasta raiz" className="flex-1 text-sm p-1.5 border border-gray-300 rounded focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none" onKeyDown={e => { if (e.key === 'Enter' && newRootFolderName.trim()) { onCreateFolder(newRootFolderName); setNewRootFolderName(''); setIsCreatingRootFolder(false); }}} />
           ...
           <button onClick={() => setIsCreatingRootFolder(true)} className="w-full flex items-center justify-center gap-2 bg-white border border-gray-200 hover:border-indigo-300 hover:text-indigo-600 text-gray-600 font-medium py-2 rounded-lg transition-colors text-sm shadow-sm"><Plus className="w-4 h-4" /> Nova Pasta Raiz</button>
         )}
         <button onClick={onOpenMethodology} className="w-full flex items-center justify-center gap-2 bg-indigo-50 border border-indigo-200 hover:bg-indigo-100 text-indigo-700 font-medium py-2 rounded-lg transition-colors text-sm shadow-sm"><GraduationCap className="w-4 h-4" /> Método e Instruções</button>
      </div>
    </div>
  );
};